...
///////////////////////////////////////////////////////////////////////////////////
///////////////////////////////////////////////////////////////////////////////////////////////
//               Mult_NonlocVector: function to multiply Nonlocal psd and vector      //
///////////////////////////////////////////////////////////////////////////////////////////////
////////
...
int Mult_NonlocalVector(LSDFT_OBJ *pLsdft, Vec *V1, Vec *V2,int Kp, int Jp, int Ip)
{
  ...
  int i, j, k, l, m, n, p, l2, at, poscnt, posIdx,LI;
  double Chi;
  ...
  for (at = 0; at < pLsdft->Ntype; at++)
      ...
      for (poscnt = 0; poscnt < pLsdft->AtomOverlap_nonlocal[at].Natoms; poscnt++) 
	{
	  ...
	  // loop over l,m,k,j,i
	  for(LI=0;LI<pLsdft->ProjectorNonlocal[at].Ndim[poscnt];LI++)
	    {
	      Chi=pLsdft->ProjectorNonlocal[at].Chi[poscnt][LI];
	      l=pLsdft->ProjectorNonlocal[at].lId[poscnt][LI];
	      //  m=pLsdft->ProjectorNonlocal[at].mId[poscnt][LI];
	      k=pLsdft->ProjectorNonlocal[at].kId[poscnt][LI];
	      j=pLsdft->ProjectorNonlocal[at].jId[poscnt][LI];
	      i=pLsdft->ProjectorNonlocal[at].iId[poscnt][LI];
	      
	      // if(rank==0)
	      // 	{
	      // 	  printf("MatMult: LI=%d poscnt=%d l=%d,m=%d,k=%d,j=%d,i=%d \n",LI,poscnt,l,m,k,j,i);
	      // 	}
	      ...
	      if((k<=Zend && k>=Zstart) && (j<=Yend && j>=Ystart) && (i<=Xend && i>=Xstart))
		{
		  ...
		  alpha[posIdx]+=Varray[k-Zstart][j-Ystart][i-Xstart]*Chi*pLsdft->psd[at].Gamma[l];
		  posIdx++;
		}// if k<=Zend
	    }// for LI
	}// for poscnt
  ...
  for (at = 0; at < pLsdft->Ntype; at++)
      ...
      for (poscnt = 0; poscnt < pLsdft->AtomOverlap_nonlocal[at].Natoms; poscnt++)
	{
	  ...
	  for(LI=0;LI<pLsdft->ProjectorNonlocal[at].Ndim[poscnt];LI++)
		...
		{
		  posIdx = pLsdft->IP_index[p];
		  V2array[k-Zstart][j-Ystart][i-Xstart]+=Chi*alpha[posIdx];
		  posIdx++;
		}// if k<=Zend
	    }// for LI
	}// for poscnt
